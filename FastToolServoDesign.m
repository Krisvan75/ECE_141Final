...
% Kambiz Shoarinejad, Feb 15, 2021 
...
% the values given in the problem set. You can obviously introduce your own
...
P_elec = (25.*Rs)/((s*Lc)+Rc+Rs); %electrical plant transfer function: Vs/Vc

% Designing the current controller
R2 = 5000 %value of resistor R2
R3 = 121290 %value of resistor R3
C1 = 1.37e-10 %value of capacitor C1
C2 = 1.39e-12 %value of capacitor C2

C_elec = (1+R3*C1*s)/(R2*s*(R3*C1*C2*s+C1+C2)) %electrical controller transfer function -Vc/Vs
...
Gp = 100.5/(s^2+100.5*s+1005012)* exp(-1.2e-5*s)     % fitted mechanical plant Gp_fit=X/F

C_mech = (-0.1)*(1+100/s)*((1+7.25e-5*s)/(1+5.203e-6)) % mechanical controller V_set/V_e   
